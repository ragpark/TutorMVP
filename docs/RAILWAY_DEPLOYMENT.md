...
API and worker builds must go through `turbo run build --filter=<app>...` (not a bare `pnpm build --filter <app>`), because the `<app>...` suffix tells Turborepo to build each app's `packages/*` workspace dependencies first via the `dependsOn: ["^build"]` rule in `turbo.json`. Every internal package publishes through `dist/` (its `package.json` `main` field), so skipping that step leaves `dist/` missing and the service crashes on start with `Cannot find module '@tutor/...'`. The web app has no internal workspace dependencies and is deployed with Root Directory set to `apps/web`, so it intentionally uses local `pnpm build` / `pnpm start` commands instead of `turbo --filter=web...`; when Railpack runs inside `apps/web`, Turborepo treats that folder as the root package rather than a workspace member, causing `No package found with name 'web' in workspace`.
...
For the same reason, `turbo` is listed as a `devDependency` in the app packages that invoke it, not just the repo root. pnpm only symlinks a package's bin into that package's own `node_modules/.bin`; with Root Directory scoped to `apps/api` or `apps/worker`, `pnpm turbo run build ...` runs from inside that folder, and if `turbo` is only declared at the workspace root it fails with `Command "turbo" not found`. Keep the `turbo` devDependency version in sync across the root and every app that invokes Turbo if you ever bump it.

On API startup, Railway runs `pnpm db:deploy && pnpm db:seed && pnpm start`. `db:deploy` uses Prisma `db push` to create/update the MVP schema for a fresh Railway PostgreSQL service, and `db:seed` creates demo users plus the Algebra Foundations knowledge graph only when it is missing. To intentionally reset and reseed the MVP dataset from the API service shell, run `FORCE_SEED=true pnpm db:seed`. If Redis is absent locally, worker behavior falls back synchronously for MVP jobs.
